...
name: getlayers
description: >-
  Use WHENEVER the user wants to build or design with GetLayers — a website or
  landing page, custom 3D scene, video background, or motion/3D on an existing
  project — or whenever they mention GetLayers, getlayers, or the GetLayers MCP.
  Drives the GetLayers MCP end-to-end so results use the real library (templates,
  scenes, compositions) instead of generic output. Load BEFORE any getlayers_* tool.
---

# Driving the GetLayers MCP (Cursor)

Upstream source (verbatim Claude copy also kept at
`.claude/skills/getlayers/SKILL.md`):
https://storage.getlayers.ai/skill/getlayers/SKILL.md

This is the **Cursor Agent** adaptation. Same GetLayers flow and non-negotiables;
tooling and environment differ as noted below.

## Cursor adaptations (vs Claude Code)

- Prefer this skill (`.cursor/skills/getlayers`) over Claude slash commands when
  working in Cursor.
- Call GetLayers tools via **CallMcpTool** after reading the tool schema under
  the enabled MCP server folder (`mcps/…/tools/getlayers_*.json`). Never invent
  argument names.
- You run setup yourself in the agent shell (install, clone, download assets,
  `yarn`/`npm`, write `.env` from `.env.example`) — do not tell the user to run
  setup commands.
- If GetLayers MCP is missing/disabled: say so, ask the user to enable it, then
  stop — do not fake library output with generic UI.
- In **this repo (GoranPortfolio / G8 Flow)**: G8 Flow brand, section order, and
  copy rules (`.cursor/rules/g8-flow-*.mdc`) win over GetLayers Style/palette
  defaults when they conflict. Use GetLayers for motion/3D/templates/assets, then
  re-skin to G8 tokens. For a pure GetLayers demo (e.g. `lumora/`), follow
  GetLayers Style fully.
- Fresh greenfield starter from GetLayers docs:
  `https://github.com/textura-agency/next16-claude-starter` — clone, strip remote
  git, `git init`, read `obsidian/` first. If the user is already in a Next/
  React project, **adapt portable HTML into that stack** instead of forcing a
  second app.
...
- **Establish the environment before materializing.** Recommend the starter, set up
  ...
  remove its git connection and init a new one (`git clone my-site && rm -rf
...
- **Show previews via the site.** Every card carries a `preview` link
  (`getlayers.ai/?layer=…`) — share it so the user watches the asset ON the site.
...
- **Anything can be a reference.** If the user names a GetLayers asset — "use
  like X" — pull it with `getlayers_source` (or its card),
